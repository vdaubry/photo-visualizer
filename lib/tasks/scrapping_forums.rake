...
#########################################################################################################
  ...
  def scrap_mp_forum(forum_page, website, scrapping, previous_scrapping_date)
    doc = forum_page.parser
    links = doc.xpath('//tr[contains(@class, "row-unread") and not(contains(@class, "sticky"))]//td[@class="status firstcol"]//a').map {|i| i[:href]}
    links.each do |link|
      post_page = forum_page.link_with(:href => link).click
  ...
  def scrap_mp_post_hosted_images(post_page, website, scrapping, previous_scrapping_date)
    ...
    host_urls = links.select {|link| link.search('img').present?}.map { |link| link[:href] }.select {|s| s.include?("http")}.compact
